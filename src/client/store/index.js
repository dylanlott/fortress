...
export const mutations = {
  ...
  FETCH_ALL_CARDS_SUCCESS (state, cards) {
    console.log('fetching cards...')
    state.lists.cards = cards
  },
  FETCH_ALL_CARDS_ERROR (state, error) {
...
export const actions = {
  ...
  async fetchAllCards ({ commit, state }) {
    try {
      commit('FETCH_ALL_CARDS_REQUEST')
      let { data } = await axios.get('/cards')
      commit('FETCH_ALL_CARDS_SUCCESS', data)
    } catch (error) {
      console.log('Error: ', error)
      commit('notification/FAILURE', error.response.data, { root: true })
      commit('FETCH_ALL_CARDS_FAILURE', error)
    }
  }
}
